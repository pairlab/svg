...
parser.add_argument('--action_space', default=4, type=int, help='total actions in action space')
...
if opt.model_dir != '':
    ...
    name = 'type=lp_cond-model=%s%dx%d-anno=%s-rnn_size=%d-predictor-posterior-prior-rnn_layers=%d-%d-%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%s-beta=%.7f%s-batch_size=%d' %\
     (opt.model, opt.image_width, opt.image_width, opt.annotations_file, opt.rnn_size, \
     opt.predictor_rnn_layers, opt.posterior_rnn_layers, opt.prior_rnn_layers, opt.n_past, \
     opt.n_future, opt.lr, opt.g_dim, opt.z_dim, opt.last_frame_skip, opt.beta, opt.name, \
     opt.batch_size)
